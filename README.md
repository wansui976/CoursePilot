...
CoursePilot是一款AI 课程视频学习工作台。它可以把课程视频转成可检索、可复习、可继续加工的学习资料：字幕、章节、笔记、摘要、课件截图、OCR 文本、脑图、练习题和课程问答。

项目适合个人学习场景，尤其是长课程、网课录屏、B 站课程和本地视频资料整理。

官网 / 介绍页：[https://wansui976.github.io/CoursePilot/](https://wansui976.github.io/CoursePilot/)

## 功能特性

- **课程库管理**：按课程文件夹组织视频，支持本地视频导入和 Bilibili URL 下载。
- **语音识别**：支持本地 whisper.cpp，也支持火山、阿里云 DashScope 等云端识别后端。
- **字幕自动纠错**：识别完成后用大模型按分段 id 批量纠错（并发可调），错别字与口语停顿更干净；也可在文稿里手动修改。
- **AI 学习资料**：基于转写文稿生成章节、概览、笔记、练习题和脑图。
- **课件与 OCR**：按画面换页自动抽取课件页，支持对当前画面或框选区域做文字识别（本地 tesseract 或阿里云 OCR）。
- **课程问答**：把课程字幕作为上下文交给大模型作答，回答按句标注 `[mm:ss]` 出处；
- **文稿搜索**：在字幕里做本地关键词匹配，命中可点击跳转。
...
导入一个视频后会进入**分阶段的处理管线**，每个阶段在「处理队列」里单独排队、单独显示进度，失败可单独续跑：
...
- 学习资料（数据库、字幕、课件图、笔记）保存在**本地设备**。
- 云端 ASR / 大模型 / OCR 需要使用**你自己的 API Key**，凭证存进系统钥匙串，不经过第三方服务器中转。
...
1. **本地依赖已安装**：至少保证 `ffmpeg`、`whisper-cli`、`tesseract`、`yt-dlp` 可用。release中可以直接下载安装包，不用单独安装依赖。
2. **准备课程来源**：可以是本地视频文件，也可以是 Bilibili / 其他视频链接,Bilibili 需要上传cookies,不然无法下载。
3. **决定是否使用纯离线模式**：
   - 只做本地转写、课件抽帧、本地 OCR，可以不配置任何云端 API。建议使用云端 API，本地 ASR 效果较差。
...
- **当前课程问答并不依赖 embeddings**：“课程问答”是把课程字幕作为上下文直接交给 LLM 回答；“文稿搜索”则是本地关键词匹配，不需要额外配置 OpenAI-compatible embeddings。
