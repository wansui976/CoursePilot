...
`tauri.conf.json` 里 `security.csp` 为 `null`。它和上一条是同一个威胁模型的两半：
注入到 WebView 的脚本拿不到密钥了，但仍能做别的事。开一条策略后要回归验证
KaTeX、markmap、本地媒体服务（`http://127.0.0.1:*`）这几处还能正常加载。

### 3. 阿里云 ASR 的 data URI 未经真机验证（可能整条后端不可用）

官方文档要求 `file_url` 是**公网可访问地址**，没说支持 base64 data URI，而我们传的
正是 data URI。字段名（qwen3 系列用单数 `file_url`、fun-asr/paraformer 用复数数组）
已按模型区分。

若服务端确实拒收 data URI，这条后端需要先把音频传到可访问的位置（如 OSS）再提交
——那是另一件工程，不是改参数能解决的。**请用真 Key 跑一次确认。**

### 4. iOS 抽音轨改动未经真机编译

阿里云那条路径在 iOS 上原来直接上传**原始视频**，读整个文件 + base64 + JSON，
1GB 的课峰值三四个 GB，会被系统杀掉。已改成先抽音轨（与 Android、与火山那条一致，
原生桥本来就支持按格式导出）。Linux 上 `cargo check` 不编译 iOS 的条件分支，
**需要在 Xcode 上过一遍**。

### 5. 大模型设置面板两条测试红着

`LlmSettingsPanel.test.tsx` 里「删除此配置」「保存 LLM 配置」两条找不到按钮。
这是随 CloudKit 那两个提交合进来的，不确定是组件有意改了结构、还是测试没跟上，
故未擅自改动。
